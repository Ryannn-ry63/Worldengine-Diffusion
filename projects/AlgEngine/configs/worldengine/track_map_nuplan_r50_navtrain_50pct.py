"""
Detection/Tracking: UniAD pipeline for object detection + map segmentation (no planning).
Key: model.type="UniAD", seg_head=PansegformerHead, 40 epochs, batch_size=1.
...
total_epochs = 40
evaluation = dict(interval=40, pipeline=test_pipeline)
runner = dict(type="EpochBasedRunner", max_epochs=total_epochs, max_keep_ckpts=1)
log_config = dict(
    interval=10, 
    hooks=[
