...
class CollisionLoss(nn.Module):
    ...
    def forward(
        self, sdc_traj_all, sdc_planning_gt, sdc_planning_gt_mask, future_gt_bbox, scene_token: str = None, frame_idx: int = None,
    ):
        # sdc_traj_all (1, 6, 2), xy coordinates, wlh format
        # sdc_planning_gt (1, 6, 3), xy coordinates, wlh format
        # sdc_planning_gt_mask (1, 6)
        # future_gt_bbox 6x[lidarboxinstance], lwh format, yx coordinate
        ...
        for i in range(n_futures):
            if len(future_gt_bbox[i].tensor) > 0:
                ...
                future_gt_bbox_corners = future_gt_bbox[i].corners[
                    # :, [0, 3, 4, 7], :2
                    :, [0, 3, 7, 4], :2     # XW: fixed the bug of the wrong box order
                ]  # (N, 8, 3) -> (N, 4, 2) only bev
                w = self.w
                l = self.h

                # updated code changing xy to yx coordinate to align with future gt bbox
                sdc_yaw = -sdc_planning_gt[0, i, 2].to(sdc_traj_all.dtype) - 1.5708
                l = self.w
                w = self.h
                
                # original code with yx coordinate
    ...
    def inter_bbox(self, corners_a, corners_b):
        ...
        intersect = max((xi1 - xi2), xi1.new_zeros(1,).to(xi1.device)) * max(
            (yi1 - yi2),
            xi1.new_zeros(
                1,
            ).to(xi1.device),
