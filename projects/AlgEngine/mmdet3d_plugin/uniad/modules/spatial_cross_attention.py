    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_lora and forward_origin==False:
            slots = self.output_proj(slots) + self.output_proj_lora(slots,i=task_idx)
        else:
...
class MSDeformableAttention3D(BaseModule):
    ...
    def __init__(
        ...
        if self.use_lora:
            lora_layer = MOELoRALinear if moe_lora else LoRALinear
            self.sampling_offsets_lora = lora_layer(embed_dims, num_heads * num_levels * num_points * 2,
                r=lora_rank, dropout=lora_drop, num_task=num_task)
            self.attention_weights_lora = lora_layer(embed_dims, num_heads * num_levels * num_points,
                 r=lora_rank, dropout=lora_drop, num_task=num_task)
