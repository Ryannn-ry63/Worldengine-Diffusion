...
class TemporalSelfAttention(BaseModule):
    ...
    def __init__(
        ...
        if self.use_lora:
            ...
            self.sampling_offsets_lora = lora_layer(embed_dims * self.num_bev_queue, num_bev_queue * num_heads * num_levels * num_points * 2,
                r=lora_rank, dropout=lora_drop, num_task=num_task)
            self.attention_weights_lora = lora_layer(embed_dims * self.num_bev_queue, num_bev_queue * num_heads * num_levels * num_points,
                 r=lora_rank, dropout=lora_drop, num_task=num_task)
            self.value_proj_lora = lora_layer(embed_dims, embed_dims, r=lora_rank, dropout=lora_drop, num_task=num_task)
            self.output_proj_lora = lora_layer(embed_dims, embed_dims, r=lora_rank, dropout=lora_drop, num_task=num_task)
        self.init_weights()
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_lora and forward_origin==False:
            output = self.output_proj(output) + self.output_proj_lora(output,i=task_idx)
        else:
