...
class HungarianAssigner_filter(BaseAssigner):
    ...
    def assign(
        ...
        for i in range(min(self.max_pos, 300 // num_gts)):
            ...
            matched_row_inds = torch.from_numpy(matched_row_inds).to(bbox_pred.device)  # (2, )
            matched_col_inds = torch.from_numpy(matched_col_inds).to(bbox_pred.device)  # (2, )
