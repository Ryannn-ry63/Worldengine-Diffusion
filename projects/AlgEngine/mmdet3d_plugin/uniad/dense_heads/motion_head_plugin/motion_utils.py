...
def nonlinear_smoother(gt_bboxes_3d, gt_fut_traj, gt_fut_traj_mask, bbox_tensor):
    ...
    fut_horizon = gt_fut_traj.shape[1]
    
    gt_fut_traj_xy_diff = np.zeros((gt_fut_traj.shape[0], fut_horizon+1, 2))  # add 1 for the diff
    ...
    for i in range(gt_fut_traj.shape[0]):
        ts = ts_limit[i]
        x_curr = [
            bbox_tensor[i, 0], bbox_tensor[i, 1], yaw_preds[i], speed_preds[i],
        ]
        ...
        if (
            ...
            if not _check_ade(traj_perturb, reference_trajectory, thres=1.5):
                ...
                traj_perturb = np.zeros((fut_horizon, 2))
