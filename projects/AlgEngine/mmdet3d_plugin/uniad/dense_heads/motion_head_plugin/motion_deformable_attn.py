...
from mmdet3d_plugin.uniad.modules.multi_scale_deformable_attn_function import (
...
class MotionTransformerAttentionLayer(BaseModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ffn_cfgs['use_lora'] = use_lora
        ffn_cfgs['lora_rank'] = lora_rank
        ...
        for ffn_index in range(num_ffns):
            if "embed_dims" not in ffn_cfgs[ffn_index]:
                ...
                # print('ffn', ffn_cfgs[ffn_index]["embed_dims"])
    ...
    def forward(
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            if layer == "self_attn":
                # input is [b, 1, mode, dim] to satisfy DCN
                query = query[:, 0]
                ...
                query = self.attentions[attn_index](
                    ...
                    task_idx=task_idx,forward_origin=forward_origin,
                    **kwargs,
                )
                attn_index += 1
                identity = query
                ...
                query = self.ffns[ffn_index](query, identity if self.pre_norm else None, task_idx=task_idx,
                                             forward_origin=forward_origin)
                ffn_index += 1
...
class MotionDeformableAttention(BaseModule):
    ...
    def __init__(
        ...
        if self.use_lora:
            ...
            self.sampling_offsets_lora = lora_layer(embed_dims, num_heads* num_steps * num_levels * num_points * 2,
                r=lora_rank, dropout=lora_drop, num_task=num_task)
            self.attention_weights_lora = lora_layer(embed_dims, num_heads * num_steps * num_levels * num_points,
                 r=lora_rank, dropout=lora_drop, num_task=num_task)
            self.value_proj_lora = lora_layer(embed_dims, embed_dims, r=lora_rank, dropout=lora_drop, num_task=num_task)
            self.output_proj_lora = lora_wrapper(self.output_proj, lora_layer, rank=lora_rank, num_task=num_task)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_lora and forward_origin==False:
            value = self.value_proj(value) + self.value_proj_lora(value,i=task_idx)
        else:
        ...
        if self.use_lora and forward_origin==False:
            # print(task_idx.shape, query.shape)
            sampling_offsets += self.sampling_offsets_lora(query,i=task_idx).view(
                bs, num_query, self.num_heads, self.num_steps, self.num_levels, self.num_points, 2
            )
            attention_weights += self.attention_weights_lora(query,i=task_idx).view(
        ...
        if reference_trajs.shape[-1] == 2:
            if isinstance(self.sample_index, list):
                reference_trajs = reference_trajs[:, :, :, self.sample_index, :, :]
            else:
                reference_trajs = reference_trajs[:, :, :, [self.sample_index], :, :]
            if bbox_results is None:
                # for BEVPlanner, querying ego traj, so in ego_coords already:
                reference_trajs_ego = reference_trajs.detach()
            else:
        ...
        if self.use_lora and forward_origin==False:
            output = peft_wrapper_forward(output, self.output_proj, self.output_proj_lora, task_idx=task_idx)
        else:
