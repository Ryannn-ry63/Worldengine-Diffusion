...
class PansegformerHead(SegDETRHead):
    ...
    def forward(self, bev_embed):
        ...
        # perform detr-like detection only, first-stage for instance segmentation

        _, bs, _ = bev_embed.shape  # HW x 1 x 256
        ...
        # memory: 1 x HW x 256
        # memory_mask: 1 x HW
        # memory_pos: 1 x HW x 256
        # query: 1 x 300 x 256
        # query_pos: 1 x 300 x 256
        # hw_lvl: H x W
        args_tuple = [memory, memory_mask, memory_pos, query, None, query_pos, hw_lvl]

        hs = hs.permute(0, 2, 1, 3)  # NumLayer x 1 x 300 x 256
        outputs_classes = []
        outputs_coords = []
        for lvl in range(hs.shape[0]):
            ...
            # regress the results based on every layer of the decoder
        ...
        outs = {
            "bev_embed": None if self.as_two_stage else bev_embed,  # HW x 1 x C
            "outputs_classes": outputs_classes,  # NumLayer x 1 x 300 x 3
            "outputs_coords": outputs_coords,  # NumLayer x 1 x 300 x 4
            "enc_outputs_class": enc_outputs_class
            if self.as_two_stage
            else None,  # None
            "enc_outputs_coord": enc_outputs_coord.sigmoid()  # None
            if self.as_two_stage
            else None,
            "args_tuple": args_tuple,
            "reference": reference,  # 1 x 300 x 4
        }
    ...
    def loss(
        ...
        # loss is for the standard bounding box detection
        ...
        # loss for the last layer, including also the segmentation mask loss
        (
            losses_cls_f,
            losses_bbox_f,
            losses_iou_f,
        ...
        # loss from the last location decoder layer
        ...
        # loss from the mask decoder (multiple layers)
        ...
        # loss from previous layers of the location decoder, only for things
        num_dec_layer = 0
        ...
        ######## extract the predicted semantic mask in BEV for prediction/planning use

        # only select the query after filtering
        cls_scores = all_cls_scores[-1]  # 1 x 300 x 3
        index_list = pos_inds_mask_list[0]
        cls_scores = cls_scores[:, index_list]  # 1 x 22 x 3
        # num_ins_categories = all_cls_scores.size(2)

        # only get the class with the highest score
        mask_label = torch.argmax(cls_scores, axis=-1)[0]  # 25

        # convert instance-level into semantic-level masks
        mask_semantic = torch.zeros(
            (self.num_things_classes, mask_preds_things.size(1), mask_preds_things.size(2)),
            dtype=mask_preds_things.dtype,
            device=mask_preds_things.device,
        )
        # print('mask_semantic', mask_semantic.size())
        mask_semantic_gt = torch.zeros(
            (self.num_things_classes, mask_preds_things.size(1), mask_preds_things.size(2)),
            dtype=mask_preds_things.dtype,
            device=mask_preds_things.device,
        )

        # loop through each instance, ignore the last one for drivable area
        for instance_index in range(mask_label.size(0)):
            lane_label = int(mask_label[instance_index])
            lane_label_gt = int(gt_labels_list[0][instance_index])

            # take the element-wise maximum between the two array
            # use sum to avoid torch.maximum to avoid GPU memory increase
            mask_semantic[lane_label] = (
                mask_semantic[lane_label] + mask_preds_things[instance_index]
            )
            mask_semantic_gt[lane_label_gt] = (
                mask_semantic_gt[lane_label_gt]
                + gt_things_masks_list[0][instance_index]
            )  # H x W

        mask_semantic = torch.cat(
            (mask_semantic, mask_preds_stuff), dim=0
        )  # 4 x H x W, lane / crossing / road boundary / drivable area
        # print('mask_semantic', mask_semantic.size())
        mask_semantic_gt = torch.cat(
            (mask_semantic_gt, gt_stuff_masks_list[0]), dim=0
        )  # 4 x H x W, lane / crossing / road boundary / drivable area, binary mask

        mask_semantic_flat = mask_semantic.view(self.num_things_classes+1, 1, -1)  # 4 x 1 x HW
        # print('mask_semantic_flat', mask_semantic_flat.size())
        mask_semantic_flat = torch.transpose(mask_semantic_flat, 0, 2)  # HW x 1 x 4
        mask_semantic_flat_gt = mask_semantic_gt.view(self.num_things_classes+1, 1, -1)  # 4 x 1 x HW
        mask_semantic_flat_gt = torch.transpose(
            mask_semantic_flat_gt, 0, 2
        )  # HW x 1 x 4

        return loss_dict, mask_semantic_flat, mask_semantic_flat_gt, mask_query_panoptic
    ...
    def loss_single_panoptic(
        ...
        # gt_stuff_labels_list: 1, int
        # gt_stuff_masks_list: 1, 1 x H x W

        # compute the standard bounding box detection loss for the last layer
        ...
        (
            ...
            loss_iou,
            loss_bbox,
            pos_inds_mask_list,  # len
            num_total_pos_thing,
        ) = self.get_filter_results_and_loss(
            cls_scores,  # 1 x 300 x 3
            bbox_preds,  # 1 x 300 x 4
            cls_scores_list,
            bbox_preds_list,
            gt_bboxes_list,
            gt_labels_list,
        ...
        ############ get the initial thing and stuff query features for mask estimation

        # get the stuff query feature
        ...
        stuff_query_pos = stuff_query_pos.unsqueeze(0).expand(BS, -1, -1)  # 1 x 1 x 256
        stuff_query = stuff_query.unsqueeze(0).expand(BS, -1, -1)  # 1 x 1 x 256

        # get the thing query feature
        len_query = max([len(pos_ind) for pos_ind in pos_inds_mask_list])
        thing_query = torch.zeros(
            [BS, len_query, dim_query], device=query.device
        )  # 1 x 15 x 256
        for i in range(BS):
        ...
        ############ 2nd-stage segmentation head with attention
        ...
        # get segmentation results for things/stuff at multiple layers of the mask heads
        ...
        # query_inter_things: len of 4, each with 1 x M x 256
        # query_inter_stuff: len of 6, each with 1 x 1 x 256
        mask_things = mask_things.squeeze(-1)  # 1 x M x HW
        mask_inter_things = torch.stack(mask_inter_things, 0).squeeze(
            -1
        )  # 4 x 1 x 15 x HW
        mask_stuff = mask_stuff.squeeze(-1)  # 1 x 1 x HW
        mask_inter_stuff = torch.stack(mask_inter_stuff, 0).squeeze(
            -1
        )  # 6 x 1 x 1 x HW

        # compose the query feature of the things/stuff from the last layer
        mask_query_panoptic = torch.cat(
            (query_inter_things[-1], query_inter_stuff[-1]), axis=1
        )  # 1 x (M+1) x 256
        # print(mask_query_panoptic.size())

        ############# to get the new bbox / cls predictions
        # loop through the batch, typically = 1
        for i in range(BS):
        ...
        mask_preds_things = torch.cat(mask_preds_things, 0)  # M x H x W
        ...
        mask_preds_stuff = torch.cat(mask_preds_stuff, 0)  # 1 x H x W
        ...
        mask_stuff_gt = torch.cat(mask_stuff_gt, 0).to(
            torch.float
        )  # 1 x H x W, 200x200

        # compute loss only for the last mask decoder layer
        ...
        # compute loss for each mask decoder layer
        ...
        return (
            # bbox-level loss in the last layer of the location decoder
            loss_cls,
            loss_bbox,
            loss_iou,
            # segmentation-level loss in all layers of the mask decoder
            loss_mask_things,
            loss_mask_stuff,
            loss_mask_things_list,
            loss_mask_stuff_list,
            # redudant bbox-level loss in the mask decoder, not used
            loss_iou_list,  # 0
            loss_bbox_list,  # 0
            loss_cls_thing_list,  # 0
            loss_cls_stuff_list,  # 0
            things_ratio,
            stuff_ratio,
            # predicted bev masks & latent feature
            mask_preds_things,
            mask_preds_stuff,
            mask_query_panoptic,
            pos_inds_mask_list,
    ...
    def forward_test(
        self,
        pts_feats=None,
        gt_lane_labels=None,    # len of 1, 1 x M+1
        gt_lane_masks=None,     # len of 1, 1 x M+1 x H x W
        img_metas=None,
        rescale=False,
    ):
        ...
        # compute IoU results between prediction and GT
        with torch.no_grad():

            # the last channel is the driveable area
            drivable_pred = results[0]["drivable"]
            try:
                drivable_gt = gt_lane_masks[0][0, -1]   # H x W
            except TypeError:
                print('Invalid input, Panseg Head')
                gt_lane_masks = [torch.zeros(1, 2, 200, 200).to(drivable_pred.device)]
                gt_lane_labels = [torch.zeros(1, 2).to(drivable_pred.device)]
            ...
            # print('drivable_pred', drivable_pred.size())
            # print('drivable_gt', drivable_gt.size())

            # merge all the instance-level (excluding the driveable area)
            # into a binary map
            lane_pred = results[0]["lane"]  # 2 x H x W
            lanes_pred = (results[0]["lane"].sum(0) > 0).int()  # H x W
            ...
            # nusc: 0 -> lane divider, 1 -> crossing, 2 -> contour
            # carla: 0 -> lane boundary, 1 -> crosswalk, 2 -> centerline, 3 -> ped walkway
            # category 0
            divider_gt = (
            ...
            # category 2, might not be there for carla
            contour_gt = (
            ...
            try:
                ...
                contour_iou = torch.zeros(1)
                contour_intersection = torch.zeros(1)
                contour_union = torch.zeros(1)
        ...
        mask_semantic_gt = torch.stack(
            [divider_gt, crossing_gt, contour_gt, drivable_gt], dim=-1
        )  # H x W x 4
        mask_semantic_flat_gt = mask_semantic_gt.view(-1, 1, 4)  # HW x 1 x 4
        ...
        for result_dict, pts_bbox in zip(bbox_list, results):
            ...
            result_dict["mask_semantic_flat"] = results[0][
                "mask_semantic_flat"
            ]  # HW x 1 x 4
            result_dict["mask_semantic_flat_gt"] = mask_semantic_flat_gt
            result_dict["mask_query_panoptic"] = results[0][
                "mask_query_panoptic"
            ]   # 1 x 101 x 256
    ...
    def forward_train(
        ...
        (
            losses_seg,
            mask_semantic_flat,
            mask_semantic_flat_gt,
            mask_query_panoptic,
        ) = self.loss(*loss_inputs, img_metas=img_metas)

        # add mapping results into the dictionary, latent feature and BEV outputs
        pred_seg_dict["mask_semantic_flat"] = mask_semantic_flat
        pred_seg_dict["mask_semantic_flat_gt"] = mask_semantic_flat_gt
        pred_seg_dict['mask_query_panoptic'] = mask_query_panoptic
    ...
    def _get_bboxes_single(
        ...
        # take a maximum of 100 thing results at testing
        ...
        if self.loss_cls.use_sigmoid:
            ...
            # only choose highest scores from the prediction, can be from the same
            # detection, e.g., a query has both high confidence in lane divider/road divider
            scores, indexes = cls_score.view(-1).topk(max_per_img)

            # get back the original ID of the query and also which class it is
    ...
    def get_bboxes(
        ...
        # taking results from the last layer of the decoder
        cls_scores = all_cls_scores[-1]  # 1 x 300 x 3
        bbox_preds = all_bbox_preds[-1]  # 1 x 300 x 4
        memory, memory_mask, memory_pos, query, _, query_pos, hw_lvl = args_tuple
        # memory: 1 x HW x 256
        # memory_mask: 1 x HW
        # memory_pos: 1 x HW x 256
        ...
        for img_id in range(len(img_metas)):
            ...
            # labels are the index of thing classes
            # bbox: 100 x 5
            ...
            stuff_query_pos = self.stuff_query.weight[
                None, :, self.embed_dims :
            ]  # 1 x 1 x 256
            ...
            attn_map = torch.cat([mask_things, mask_stuff], 1)  # 1 x 101 x HW x 1
            attn_map = attn_map.squeeze(-1)  # BS, NQ, HW
            ...
            mask_pred = attn_map.reshape(-1, *hw_lvl[0])  # 101 x H x W
            ...
            # only the last one contribute to the stuff classes
            drivable_list.append(masks_all[-1] > 0.5)
            masks_all = masks_all[: -self.num_stuff_classes]    # M x H x W
            ...
            # go through each query to fill the thing BEV, not including the stuff query
            for i, scores in enumerate(scores_all):

                # filter things that are below the confidence threshold
            ...
            mask_query_panoptic_list.append(mask_query_panoptic)

        # print('lane_score_list', lane_score_list[0].size())

        # getting the explicit mapping results to pass to downstream tasks
        driveable_soft = score_list[0][-1]  # H x W
        driveable_soft = torch.unsqueeze(driveable_soft, dim=0)  # 1 x H x W
        mask_semantic = torch.cat((lane_score_list[0], driveable_soft), axis=0) # 3 x H x W
        # 4 x H x W, lane / crossing / road boundary / drivable area
        mask_query_panoptic = mask_query_panoptic_list[0]
            ...
            results.append(
                {
                    ...
                    "bbox": bbox_list[i],  # 100 x 5
                    "segm": seg_list[i],  # 100 x H x W, binary mask
                    "labels": labels_list[i],  # 100
                    "panoptic": panoptic_list[i],
                    "score_list": score_list[i],  # 101 x H x W, soft
                    # semantic-level results
                    "drivable": drivable_list[i],  # H x W, binary mask
                    "lane": lane_list[i],  # 3 X H x W, binary mask
                    "lane_score": lane_score_list[i],  # 3 X H x W, soft
                    "stuff_score_list": stuff_score_list[i],  # int
                    "mask_semantic_flat": mask_semantic_flat,  # HW x 1 x 4
                    "mask_query_panoptic": mask_query_panoptic,  # 1 x 101 x 256
                }
            )
