...
class MotionHead(BaseMotionHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bev_map_option = bev_map_option
        self.logging = False
        ...
        # add bev_map_interaction
        transformerlayers["bev_map_option"] = self.bev_map_option
        transformerlayers["use_lane_query"] = self.use_lane_query
        transformerlayers["use_map_query"] = self.use_map_query
        transformerlayers["bev_map_interaction"] = self.bev_map_interaction
    ...
    def get_additional_input_from_upstream(
        ...
        if (
            ...
            if self.bev_map_gt:
                ...
                if self.logging:
                    print("\n using mapping BEV GT in prediction")
            # extract mapping results
            else:
                map_bev = outs_seg["mask_semantic_flat"]  # HW x 1 x 4
                if self.logging:
                    print("\n using mapping BEV outputs in prediction")

        # Not using mapping BEV
        else:
            map_bev = None
            if self.logging:
                print("\n NOT using mapping BEV in prediction")
    ...
    def forward_train(
        ...
        outs_motion = self(
            ...
            track_bbox_results=track_boxes,
        ...
        # extract the last query for the ego vehicle
        ...
        # take the query for other agents
        outs_motion["traj_query"] = outs_motion["traj_query"][
            :, :, :-1
    ...
    def forward_test(self, bev_embed, outs_track={}, outs_seg={}):
        ...
        # combine with the ego sdc query embedding
        track_query = torch.cat(
            [track_query, outs_track["sdc_embedding"][None, None, None, :]], dim=2
        )
        sdc_track_boxes = outs_track["sdc_track_bbox_results"]
    ...
    def forward(
        ...
        inter_states, inter_references = self.motionformer(
            track_query=track_query,  # B, A_track, D
        ...
        for lvl in range(inter_states.shape[0]):
            ...
            # i.e., predicting the delta
            tmp[..., :2] = torch.cumsum(tmp[..., :2], dim=3)  # 1 x N x M x F x 5
        ...
        outs = {
            "all_traj_scores": outputs_traj_scores,  # 3 x 1 x 5 x 6
            "all_traj_preds": outputs_trajs,  # 3 x 1 x 5 x 6 x 12 x 5
            "valid_traj_masks": valid_traj_masks,  # 1 x 5
            "traj_query": inter_states,  # 3 x B x A x 6 x 256
            "track_query": track_query,  # B x A x 256
            "track_query_pos": track_query_pos,  # B x A x 256
        }
    ...
    def compute_matched_gt_traj(
        ...
        Args:
        gt_fut_traj: Ground truth future trajectories, List[torch.Tensor] with a len of C (# cams): N x T x 2
        gt_fut_traj_mask: Ground truth future trajectory masks, List[torch.Tensor] with a len of C (# cams): N x T x 2
        all_matched_idxes (List[torch.Tensor]): A list of tensors containing the matched indexes for each image in the batch.
        track_bbox_results: List[List[LiDARInstance3DBoxes]], with a len of C x M:
        ...
        for i in range(num_imgs):
            matched_gt_idx = all_matched_idxes[i]  # (1, ), index
            valid_traj_masks = matched_gt_idx >= 0  # (1, ), boolean
            matched_gt_fut_traj = gt_fut_traj[i][matched_gt_idx][
                valid_traj_masks
            ]  # 1 x T x 2
            ...
            if self.use_nonlinear_optimizer:
                ...
                # TODO: why use index 0
                bboxes: torch.Tensor = track_bbox_results[i][0].tensor.to(
                    matched_gt_fut_traj.device
                )  # on GPU, 1 x 9
                # print(bboxes.size())
                bboxes = bboxes[valid_traj_masks]
                matched_gt_bboxes_3d = gt_bboxes_3d[i][-1].tensor.to(
                    matched_gt_fut_traj.device
                )
                matched_gt_bboxes_3d = matched_gt_bboxes_3d[matched_gt_idx[:-1]][
                    valid_traj_masks[:-1]
                ]
