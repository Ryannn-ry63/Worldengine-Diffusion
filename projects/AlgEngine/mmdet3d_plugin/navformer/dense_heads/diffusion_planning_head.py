...
class DiffusionPlanningHead(nn.Module):
    ...
    The dict returned by ``forward`` contains the keys NavFormer needs
    (``trajectory`` of shape ``(B, 40, 3)`` and ``selected_indices`` indexing
    into the 8192-vocabulary), plus extra intermediate tensors consumed by
    ``loss``.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if vocab_path is None:
            raise ValueError("`vocab_path` must be provided for test-time vocab snapping.")
        ...
        # (i) 8192-vocab used for test-time snapping back to PDM-score indices
        vocab = np.load(vocab_path)
        self.vocab_8192 = nn.Parameter(
            torch.tensor(vocab, dtype=torch.float32),
            requires_grad=False,
        )
    ...
    def _snap_to_vocab(self, traj_8):
        """Nearest-neighbour snap to the 8192-vocab to produce ``selected_indices``.

        traj_8: (B, 8, 3); returns (B,) int64 indices into ``self.vocab_8192``.
        """
        # vocab is (L=8192, 40, 3); downsample horizon by 5 (4::5) to align with
        # the 8 waypoints used by the rest of the framework.
        vocab_xy = self.vocab_8192[None, :, 4::5, :2]  # (1, L, 8, 2)
        target_xy = traj_8[:, None, :, :2]             # (B, 1, 8, 2)
        dist = torch.linalg.norm(vocab_xy - target_xy, dim=-1).sum(-1)  # (B, L)
    ...
    def _forward_train(self, bev_feature, ego_query, agents_query, status_token):
        ...
        return {
            ...
            "selected_indices": self._snap_to_vocab(best_reg),
