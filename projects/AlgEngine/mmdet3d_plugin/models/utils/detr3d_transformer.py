...
class Detr3DTransformer(BaseModule):
    ...
    def forward(
        self, mlvl_feats, mlvl_point_feats, query_embed, reg_branches=None, **kwargs
    ):
        ...
        **kwargs,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if point_feats is not None:
                reference_points_xy = (reference_points_input[..., :2] - 0.5) * 2
                # reference_points_xy = torch.cat([reference_points_xy[..., 1:2], reference_points_xy[..., 0:1]], -1)
                reference_points_xy = reference_points_xy.unsqueeze(1)
                for point_feat in point_feats:
                    point_feat_for_reference_points = F.grid_sample(
                        point_feat, reference_points_xy
                    )  # (batch_size, embded_dims, num_query)
                    output = output + point_feat_for_reference_points.squeeze(2).permute(
                        2, 0, 1
                    )
