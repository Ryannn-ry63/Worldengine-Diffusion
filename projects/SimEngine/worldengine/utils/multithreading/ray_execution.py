...
from worldengine.utils.multithreading.worker_pool import Task
...
def _ray_map_items(task: Task, *item_lists: Iterable[List[Any]], log_dir: Optional[Path] = None) -> List[Any]:
    ...
    fn = task.fn
    # Pre-serialize all arguments into the object store so that .remote() calls
    # only pass lightweight ObjectRefs, allowing all tasks to be scheduled at once.
    item_refs = [
        tuple(ray.put(item) for item in items)
    ...
    if isinstance(fn, partial):
        ...
        object_ids = [remote_fn.remote(*refs, **args) for refs in item_refs]
    else:
        fn = wrap_function(fn, log_dir=log_dir)
        remote_fn = ray.remote(fn).options(num_gpus=task.num_gpus, num_cpus=task.num_cpus)
        object_ids = [remote_fn.remote(*refs) for refs in item_refs]
