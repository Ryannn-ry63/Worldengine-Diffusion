"""
Types of different componenets in the simulator.
"""
...
class WorldEngineObjectType:
    ...
    def is_white_line(cls, line):
        return WorldEngineObjectType.is_road_line(line) and not WorldEngineObjectType.is_yellow_line(line)
    ...
    def is_stop_sign(cls, type):
        return type == WorldEngineObjectType.STOP_SIGN

    @classmethod
    def is_speed_bump(cls, type):
        return type == WorldEngineObjectType.SPEED_BUMP

    @classmethod
    def is_driveway(cls, type):
        return type == WorldEngineObjectType.DRIVEWAY

    @classmethod
    def is_crosswalk(cls, type):
        return type == WorldEngineObjectType.CROSSWALK
    ...
    def simplify_light_status(cls, status: str):
        ...
        if status in [cls.LANE_STATE_UNKNOWN, cls.LANE_STATE_FLASHING_STOP, cls.LIGHT_UNKNOWN, None]:
            ...
            logger.warning("TrafficLightStatus: {} is not WorldEngineObjectType".format(status))
            return cls.LIGHT_UNKNOWN
    ...
    def __init__(self, type=None):
        ...
        self.type = WorldEngineObjectType.UNSET
        if type is not None:
            self.set_type(type)

    def set_type(self, type):
        if type in WorldEngineObjectType.__dict__.values():
            # Do something if type matches one of the class variables
            self.type = type
        else:
            raise ValueError(f"'{type}' is not a valid WorldEngineObjectType.")
